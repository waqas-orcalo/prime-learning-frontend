...
export interface LearningSupportForm {
  ...
  attachmentUrl: string; attachmentName: string; monthlyReview: string
  threeMonthlyReview: string; changesNotes: string; activityTracker: string
  reasonForStopping: string; tutorConfirmationA: string; tutorConfirmationB: string
  learnerConfirmation: string; learnerSigned: boolean; learnerSignedAt: string | null
  trainerSigned: boolean; trainerSignedAt: string | null; isDeleted: boolean
  createdAt: string; updatedAt: string
}
export interface CreateLearningSupportPayload {
  learnerId: string; trainerId?: string; attachmentUrl?: string; attachmentName?: string
  monthlyReview?: string; threeMonthlyReview?: string; changesNotes?: string
  activityTracker?: string; reasonForStopping?: string; tutorConfirmationA?: string
  tutorConfirmationB?: string; learnerConfirmation?: string
  learnerSigned?: boolean; trainerSigned?: boolean
}
...
interface PaginatedResponse { statusCode: number; message: string; data: LearningSupportForm[]; pagination: { total: number; page: number; limit: number; pages: number } }
interface SingleResponse { statusCode: number; message: string; data: LearningSupportForm }
...
export function useLearningSupportList(params: ListLearningSupportParams = {}) {
  ...
  return useQuery<PaginatedResponse>({
    ...
    queryFn: () => {
      ...
      return apiFetch(`/forms/learning-support${qs.toString() ? `?${qs}` : ''}`, token)
    },
...
export function useLearningSupport(id: string) {
  const { data: session } = useSession()
  const token = (session?.user as any)?.accessToken as string | undefined
  return useQuery<SingleResponse>({ queryKey: [...KEY, id], queryFn: () => apiFetch(`/forms/learning-support/${id}`, token), enabled: !!token && !!id })
}
export function useCreateLearningSupport() {
  ...
  return useMutation<SingleResponse, Error, CreateLearningSupportPayload>({
    mutationFn: (payload) => apiFetch('/forms/learning-support', token, { method: 'POST', body: JSON.stringify(payload) }),
...
export function useUpdateLearningSupport() {
  ...
  return useMutation<SingleResponse, Error, { id: string } & Partial<CreateLearningSupportPayload>>({
    mutationFn: ({ id, ...payload }) => apiFetch(`/forms/learning-support/${id}`, token, { method: 'PATCH', body: JSON.stringify(payload) }),
...
export function useDeleteLearningSupport() {
  ...
  return useMutation<{ statusCode: number; message: string; data: null }, Error, string>({
    mutationFn: (id) => apiFetch(`/forms/learning-support/${id}`, token, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: KEY }),
  })
}
export function useSignLearningSupport() {
  ...
  return useMutation<SingleResponse, Error, { id: string; role: 'learner' | 'trainer' }>({
    mutationFn: ({ id, role }) => apiFetch(`/forms/learning-support/${id}/sign`, token, { method: 'PATCH', body: JSON.stringify({ role }) }),
    onSuccess: (_d, vars) => { qc.invalidateQueries({ queryKey: KEY }); qc.invalidateQueries({ queryKey: [...KEY, vars.id] }) },
  })
}
