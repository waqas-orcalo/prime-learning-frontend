...
import { useCreateLearningSupport } from '@/hooks/use-learning-support'
...
const INIT: FormState = {
  attachmentName: '', monthlyReview: 'No/False', threeMonthlyReview: 'No/False',
  changesNotes: '', activityTracker: '', reasonForStopping: '',
  tutorConfirmationA: 'No/False', tutorConfirmationB: 'No/False',
  learnerConfirmation: 'No/False', learnerSigned: false, trainerSigned: false,
}

function CreateLearningSupportInner() {
  const router = useRouter()
  const { data: session } = useSession()
  const [form, setForm] = useState<FormState>(INIT)
  const [error, setError] = useState('')

  const createMutation = useCreateLearningSupport()
    ...
    createMutation.mutate(
      {
        ...
        monthlyReview:      form.monthlyReview,
        threeMonthlyReview: form.threeMonthlyReview,
        changesNotes:       form.changesNotes,
        activityTracker:    form.activityTracker,
        reasonForStopping:  form.reasonForStopping,
        tutorConfirmationA: form.tutorConfirmationA,
        tutorConfirmationB: form.tutorConfirmationB,
        learnerConfirmation: form.learnerConfirmation,
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 860, ...FF }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <h1 style={font(22, 600)}>Create 5.Learning Support Form</h1>
      </div>
        ...
        <div style={{ padding: '16px', borderBottom: '1px solid rgba(28,28,28,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px', background: '#fafafa', border: '1px dashed rgba(28,28,28,0.15)', borderRadius: 8 }}>
            ...
            <span style={font(12, 400, '#888')}>
              {form.attachmentName || 'Nothing is attached. Please Attach Learning Support Plan'}
            </span>
            <label style={{ marginLeft: 'auto', padding: '5px 14px', background: '#1c1c1c', color: '#fff', borderRadius: 6, cursor: 'pointer', ...font(12, 500, '#fff') }}>
              <input
                ...
                onChange={e => {
                  ...
                  if (file) setForm(f => ({ ...f, attachmentName: file.name }))
                }}
              />
              Attach file
            </label>
...
export default function Page() { return <Suspense><CreateLearningSupportInner /></Suspense> }
