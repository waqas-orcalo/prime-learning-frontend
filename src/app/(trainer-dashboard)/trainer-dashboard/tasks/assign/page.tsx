...
interface Learner { _id: string; firstName: string; lastName: string; email: string; role: string }

function CheckboxIcon({ checked }: { checked: boolean }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" style={{ flexShrink: 0 }}>
      <rect x="1" y="1" width="16" height="16" rx="4"
        stroke={checked ? '#1c1c1c' : 'rgba(28,28,28,0.25)'} strokeWidth="1.4"
        fill={checked ? '#1c1c1c' : 'transparent'} />
      {checked && <path d="M4.5 9l3 3 6-6" stroke="white" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>}
    </svg>
  )
}
...
function AssignTaskInner() {
  const router = useRouter()
  const { data: session } = useSession()
  const token = (session?.user as any)?.accessToken as string | undefined

  const { data: learnersData, isLoading: learnersLoading } = useTrainerLearners({ limit: 200 })
  const myLearners: Learner[] = (learnersData?.data ?? []) as Learner[]
  ...
  const selectedLearner = myLearners.find(l => l._id === selectedLearnerId) ?? null
  ...
  const handleAssign = async () => {
    if (!title.trim() || !selectedLearner) return
    setSubmitError('')
    setSubmitting(true)
    try {
      await apiFetch('/tasks', token, {
        ...
        body: JSON.stringify({
          ...
          assignedTo: selectedLearner._id,
        }),
      })
      setSuccess(true)
    } catch (err: any) {
        ...
        <div style={{ ...card, textAlign: 'center' }}>
          <div style={{ ...cardBody, padding: '48px 32px' }}>
            ...
            <div style={font(14, 400, 'rgba(28,28,28,0.6)', { marginBottom: '6px' })}>
              <strong>"{title}"</strong> has been assigned to {selectedLearner.firstName} {selectedLearner.lastName}
            </div>
            <div style={font(13, 400, 'rgba(28,28,28,0.45)', { marginBottom: '28px' })}>{selectedLearner.email}</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center', marginBottom: '28px' }}>
              {primaryMethod && (
                <span style={{ backgroundColor: '#e5ecf6', borderRadius: '6px', padding: '4px 10px', ...font(12, 500, '#1c1c1c') }}>
                  {primaryMethod}
                </span>
              )}
              {reference && (
                <span style={{ backgroundColor: '#f0fdf4', borderRadius: '6px', padding: '4px 10px', ...font(12, 500, '#16a34a') }}>
                  Ref: {reference}
                </span>
              )}
              {dueDate && (
                <span style={{ backgroundColor: 'rgba(28,28,28,0.06)', borderRadius: '6px', padding: '4px 10px', ...font(12, 500, '#1c1c1c') }}>
                  Due: {new Date(dueDate).toLocaleDateString('en-GB')}
                </span>
              )}
            </div>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
              <button onClick={resetForm} style={{ ...font(14, 500, '#1c1c1c'), border: '1px solid rgba(28,28,28,0.2)', borderRadius: '8px', padding: '8px 20px', height: '36px', backgroundColor: 'transparent', cursor: 'pointer' }}>
                Assign Another
              </button>
              <button onClick={() => router.push('/trainer-dashboard/tasks')} style={{ ...font(14, 500, '#fff'), backgroundColor: '#1c1c1c', border: 'none', borderRadius: '8px', padding: '8px 20px', height: '36px', cursor: 'pointer' }}>
                View Tasks
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{ maxWidth: '780px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '28px' }}>
        {[{ num: 1, label: 'Select Learner' }, { num: 2, label: 'Task Details' }].map((s, i) => (
      ...
      {step === 1 && (
        <div style={card}>
          <div style={cardHeader}>
            <span style={font(16, 600, '#1c1c1c')}>Select Your Learner</span>
          </div>
          <div style={cardBody}>
            <div style={{ ...font(13, 400, 'rgba(28,28,28,0.5)'), marginBottom: '24px' }}>
              Choose one of your assigned learners to assign a task to them.
            </div>

            {learnersLoading ? (
              <div style={{ padding: '20px 0', textAlign: 'center', ...font(14, 400, 'rgba(28,28,28,0.4)') }}>
                Loading your learners…
              </div>
            ) : myLearners.length === 0 ? (
              <div style={{ backgroundColor: '#fef9c3', border: '1px solid #fbbf24', borderRadius: '8px', padding: '14px 16px', ...font(13, 400, '#92400e') }}>
                No learners are currently assigned to you. Please contact your administrator to have learners assigned.
              </div>
            ) : (
              <>
                ...
                <div style={{ display: 'flex', gap: '10px', marginBottom: '8px' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...font(14, 500, '#fff'),
                      backgroundColor: selectedLearnerId ? '#1c1c1c' : 'rgba(28,28,28,0.3)',
                      border: 'none', borderRadius: '8px',
                      padding: '0 20px', height: '40px',
                      cursor: selectedLearnerId ? 'pointer' : 'not-allowed',
                ...
                <div style={{ ...font(12, 400, 'rgba(28,28,28,0.4)') }}>
                  {myLearners.length} learner{myLearners.length !== 1 ? 's' : ''} available
                </div>
              </>
            )}
          </div>
      ...
      {step === 2 && selectedLearner && (
        <>
          {/* Learner info banner */}
          <div style={{ ...card, boxShadow: 'none', border: '1px solid #bbf7d0', backgroundColor: '#f0fdf4', marginBottom: '16px' }}>
            <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#1c1c1c', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={font(16, 600, '#fff')}>{selectedLearner.firstName[0].toUpperCase()}</span>
              </div>
              <div>
                <div style={font(14, 600, '#1c1c1c')}>{selectedLearner.firstName} {selectedLearner.lastName}</div>
                <div style={font(13, 400, 'rgba(28,28,28,0.6)')}>{selectedLearner.email} · {selectedLearner.role}</div>
              </div>
              <button
                onClick={() => setStep(1)}
            ...
            <div style={{ ...cardBody, display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 160px', gap: '16px' }}>
                ...
                <div>
                  ...
                  <input value={reference} onChange={e => setReference(e.target.value)} placeholder="e.g. FSE1" style={inputStyle} />
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', backgroundColor: 'rgba(28,28,28,0.02)', border: '1px solid rgba(28,28,28,0.1)', borderRadius: '10px', padding: '16px' }}>
                  {METHOD_OPTIONS.map(method => (
                    <button key={method} type="button" onClick={() => toggleSecondaryMethod(method)}
                      style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', textAlign: 'left' }}>
                      <CheckboxIcon checked={secondaryMethods.includes(method)} />
                      <span style={font(13, secondaryMethods.includes(method) ? 500 : 400)}>{method}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                ...
                border: 'none', borderRadius: '8px', padding: '8px 24px', height: '36px',
                cursor: submitting || !title.trim() ? 'not-allowed' : 'pointer',
              }}
            >
              {submitting ? 'Assigning…' : `Assign Task to ${selectedLearner.firstName}`}
            </button>
          </div>
        </>
      )}
...
export default function TrainerAssignTaskPage() {
  return (
