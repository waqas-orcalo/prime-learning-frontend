...
// Derive a pseudo-progress from createdAt (demo — replace with real progress API when available)
function pseudoProgress(createdAt: string): number {
  const ms = Date.now() - new Date(createdAt).getTime()
  const days = ms / (1000 * 60 * 60 * 24)
  return Math.min(Math.round((days / 365) * 80), 95) // up to 95% over a year
}

function ProgressRow({ learner, index }: { learner: any; index: number }) {
  const fullName  = `${learner.firstName} ${learner.lastName}`
  const progress  = pseudoProgress(learner.createdAt)
...
function ProgressInner() {
  ...
  const onTrack = learners.filter(l => pseudoProgress(l.createdAt) >= 60).length
  const behind  = learners.filter(l => { const p = pseudoProgress(l.createdAt); return p >= 30 && p < 60 }).length
  const atRisk  = learners.filter(l => pseudoProgress(l.createdAt) < 30).length
    ...
    <div style={{ maxWidth: 1100, ...FF }}>
      ...
      <div style={{ border: '1px solid rgba(28,28,28,0.1)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '12px 18px', background: '#fafafa', borderBottom: '1px solid rgba(28,28,28,0.08)' }}>
          <span style={font(13, 600)}>All Learners</span>
        </div>
        ...
        {isLoading
          ...
          : learners.length === 0
            ? <p style={{ ...font(14, 400, '#aaa'), textAlign: 'center', padding: '40px 0' }}>No learners found.</p>
            : learners.map((l, i) => <ProgressRow key={l._id} learner={l} index={i} />)
        }
      </div>
    </div>
  )
